            ...
            results.map((result, index) => {
              ...
              if (options.format === 'detailed') {
                return pipe(
                  ...
                  Effect.flatMap(() =>
                    Effect.log(
                      `   📂 Type: ${doc.doc_type} | 🏷️ Category: ${doc.metadata.category}`,
                    )
                  ),
                  Effect.flatMap(() =>
                    Effect.log(
                      `   📅 Date: ${formattedDate} | ⭐ Priority: ${doc.metadata.priority}`,
                    )
                  ),
...
export const searchByTypeCommand = (
  ...
  pipe(
    ...
    Effect.flatMap(() =>
      globalSearchCommand(projectPath, query, {
        type: [type],
    ...
    Effect.flatMap((response) => {
      ...
      for (const result of response.results) {
        ...
        if (doc.metadata.category) {
          stats.byCategory[doc.metadata.category] = (stats.byCategory[doc.metadata.category] || 0) +
            1
        }
