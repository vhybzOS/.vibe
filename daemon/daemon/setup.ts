...
export const installSystemdService = (serviceContent: string) =>
  pipe(
    ...
    Effect.tap((servicePath) => Effect.log(`📝 Systemd service installed at ${servicePath}`)),
    Effect.tap(() => Effect.log('💡 Enable with: systemctl --user enable vibe-daemon.service')),
    Effect.tap(() => Effect.log('🚀 Start with: systemctl --user start vibe-daemon.service')),
  )
