...
describe('⚡ Performance Benchmarks', () => {
  ...
  describe('🔍 Tool Detection Performance', () => {
    ...
    it('📊 should scale linearly with project size', async () => {
      ...
      console.log('📈 Scaling results:', results.map((r, i) => `${testSizes[i]} files: ${r.toFixed(2)}ms`))
  ...
  describe('📋 Schema Validation Performance', () => {
    it('⚡ should validate schemas quickly', () => {
      ...
      console.log(`🔬 Schema validation: ${avgTime.toFixed(3)}ms average (${iterations} iterations)`)
    })
    ...
    it('🏋️ should handle large schemas efficiently', () => {
      ...
      assert(validationTime < 10, `Large schema validation took ${validationTime.toFixed(2)}ms (should be < 10ms)`)
  ...
  describe('💾 File System Performance', () => {
    it('📁 should handle many files efficiently', async () => {
      ...
      try {
        ...
        assert(createTime < 1000, `File creation took ${createTime.toFixed(2)}ms (should be < 1000ms)`)
