...
const handler = async (req: Request): Promise<Response> => {
  ...
  switch (url.pathname) {
    case '/api/secrets':
      return new Response(JSON.stringify({ success: true, message: 'Secret saved in test daemon.' }), { status: 200, headers: { 'Content-Type': 'application/json' } });

    case '/api/discovery/start':
      return new Response(JSON.stringify(DUMMY_DISCOVERY_RESPONSE), { status: 200, headers: { 'Content-Type': 'application/json' } });

    case '/api/events':
       return new Response('data: {"type":"discovery","progress":{"current":"Completed"}}\n\n', { status: 200, headers: { 'Content-Type': 'text/event-stream' } });

    case '/status':
      return new Response(JSON.stringify(DUMMY_STATUS_RESPONSE), { status: 200, headers: { 'Content-Type': 'application/json' } });

    case '/shutdown':
      return new Response(JSON.stringify({ message: 'Test daemon shutdown initiated.' }), { status: 200, headers: { 'Content-Type': 'application/json' } });
