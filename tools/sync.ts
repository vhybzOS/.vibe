...
import { readTextFile, writeTextFile, ensureDir } from '../lib/effects.ts'
  ...
  pipe(
    Effect.all(
      detectedTools.map(tool => 
        syncSingleTool(projectPath, tool.tool, rules)
      )
    ),
    Effect.map(results => ({
    ...
    Effect.tap(summary =>
      logWithContext('Sync', `Completed: ${summary.summary.successful}/${summary.summary.total} tools`)
    )
  )
